...
sessionrestore_session_restore() {
    ...
    else
    {
        ...
        SessionRestoreMenu := Menu()
        SessionRestoreMenu.Add(label, _sessionrestore_nop)
        SessionRestoreMenu.Disable(label)

        for i, name in layouts
            SessionRestoreMenu.Add(name, _sessionrestore_session_restore)

        SessionRestoreMenu.Add()
        SessionRestoreMenu.Add("Cancel", _sessionrestore_nop)
        SessionRestoreMenu.Show()
    }
}
...
_sessionrestore_session_restore(layout_name, *) {
    ...
    for windex, win in window_list {
        ...
        if (SessionRestore_ShowProgress) {
            iprogress := (A_Index / window_list.Length) * 100.0
            progress_text := "Preparing ...`n" A_Index "/" window_list.Length " " win.proc_name
    ...
    for windex, win in window_list {
        ...
        if (SessionRestore_ShowProgress) {
            iprogress := (A_Index / window_list.Length) * 100.0
            progress_text := "Arranging ...`n" A_Index "/" window_list.Length " " win.proc_name
            a2tip(progress_text ":" iprogress)
        }
